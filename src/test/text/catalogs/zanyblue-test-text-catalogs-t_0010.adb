...
procedure T_0010 (R : in out AUnit.Test_Cases.Test_Case'Class) is
   ...
   begin
      R.Assert (Get_Key (Catalog, 4) = "nosuchkey", "Exception not raised!");
   exception
...
begin
   ...
   R.Assert (Get_Key (Catalog, 1) = "mykey1", "Expected key name mykey1");
   Add (Catalog, "myfac1", "mykey2", "My Message", Locale);
   R.Assert (Get_Key (Catalog, 2) = "mykey2", "Expected key name mykey2");
   Add (Catalog, "myfac2", "mykey3", "My Message", Locale);
   R.Assert (Get_Key (Catalog, 3) = "mykey3", "Expected key name mykey3");
   Check_Extra_Indexes;
end T_0010;
