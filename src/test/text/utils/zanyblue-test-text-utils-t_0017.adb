...
procedure T_0017 (R : in out AUnit.Test_Cases.Test_Case'Class) is
   ...
   procedure Check_Day (Data : Day_Result_Type) is
      Day_Name : constant Day_Type := Day_In_Week (Data.Day,
                                                   ...
                                                   Data.Year);
   begin
      R.Assert (Day_Name = Data.Day_Name,
                 "Incorrect day calcaulated: "
               & Day_Number'Image (Data.Day)
               & "-"
               & Month_Number'Image (Data.Month)
               & "-"
               & Year_Number'Image (Data.Year)
               & ": "
               & Day_Type'Image (Day_Name)
               & " /= "
               & Day_Type'Image (Data.Day_Name));
   end Check_Day;
